...
class OcrExecutor(
    ...
    override fun run() {
        ...
        task.ocrProgress.status = Status.STARTED
        ...
        for (document in task.inDocuments) {
            ...
            val outFile = Path.of(ocrProperties.taskPath, task.id.toString(), "output", UUID.randomUUID().toString()).toFile()
        ...
        task.ocrProgress.status = Status.FINISHED
        taskRepository.save(task)
    }
}